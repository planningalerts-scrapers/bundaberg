...
function accept_terms_get_cookies($terms_url) {
    ...
    preg_match('/<input type="hidden" name="__VIEWSTATEGENERATOR" id="__VIEWSTATEGENERATOR" value="(.*)" \/>/', $terms_response, $viewstategenerator_matches);
    $viewstategenerator = $viewstategenerator_matches[1];
    ...
    $postfields['ctl00$cphContent$ctl00$btnOk'] = 'Agree';
...
for ($i = 1; $i <= $NumPages; $i++) {
    ...
    foreach ($dataset as $record) {
        ...
        $application = array (
            ...
            'address'           => preg_replace('/\s+/', ' ', $tempstr[0]) . ", QLD, Australia",
